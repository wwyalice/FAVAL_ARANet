...
ax.plot([np.min(data_X)-1, np.max(data_Y)+1],
           [np.min(data_X)-1, np.max(data_Y)+1], color='#da82a3')

ax.set_ylabel('Predicted Values', fontsize=9)
ax.set_xlabel('True Values', fontsize=9)
plt.savefig("./data/MXene/nested-CV_ARAnet_{}_{}_fold.png".format(Projected_target, k))
plt.show()