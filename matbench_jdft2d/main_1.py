...
for train_index, val_index in kf.split(init):
    ...
    num_epochs = 500
    best_loss_rg_mae = 1000
    best_loss_rg_mse = 1000
    best_data = None
    best_model = None
...
print("Best [MAE] of test set for {}-fold cross-validation: ".format(k),
      [float(test_best_MAE)])
# with open("./data/{}_diff_a_h_{}_new.csv".format(Projected_target, k), "a") as fp:
#     fp.write("{},{},{},{},{},{}\n".format(h_s, a, float(test_best_MAE), float(test_best_MSE),
#                                           np.mean(test_loss, axis=0)[0], np.mean(test_loss, axis=0)[1]))
...
plt.savefig("./data/FAVAL_ARAnet_test_{}_{}_fold_1.png".format("matbench_jdft2d", k))
plt.show()